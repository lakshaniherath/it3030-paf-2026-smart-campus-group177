...
const TicketListPage = () => {
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] p-6 lg:p-10 text-left">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
                    <div>
                        <h1 className="text-4xl font-black text-slate-900 tracking-tight">Maintenance</h1>
                        <p className="text-slate-500 font-medium mt-1">Manage and track campus equipment issues</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {filteredTickets.length > 0 ? (
                                ...
                                <div className="p-8 pb-10">
                                    <div className="flex justify-between items-center mb-6">
                                        <div className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-white ${
                                            ticket.status === 'REJECTED' ? 'bg-rose-500' : 'bg-indigo-600'
                                        }`}>
                                            {ticket.status}
                                        </div>
                                        <span className="text-slate-300 text-[11px] font-black uppercase">#{String(ticket.id).slice(-4)}</span>
                                    </div>

                                    {/* Description Fix - Truncate long text */}
                                    <h3 
                                    ...
                                    <div className="space-y-4 mb-8">
                                        ...
                                        <div className="flex items-center gap-4 text-slate-600">
                                            ...
                                            <div>
                                                <p className="text-[10px] uppercase font-black text-slate-400 leading-none mb-1">Reporter</p>
                                                ...
                                                {ticket.imageUrls.map((url, index) => (
                                                    <div key={index} className="relative group">
                                                        <img 
                                                            ...
                                                            alt="incident" 
                                                            className="w-32 h-32 object-cover rounded-[24px] border-4 border-white shadow-md group-hover:scale-105 transition-transform cursor-zoom-in"
                                    ...
                                    <div className="space-y-4 pt-8 border-t-2 border-slate-50">
                                        <div className="flex flex-col gap-2">
                                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Change Status</label>
                                            <select 
                                                className="w-full bg-slate-100 border-none text-sm font-black text-slate-700 rounded-2xl p-4 outline-none focus:ring-2 focus:ring-blue-400 appearance-none"
                                                value={ticket.status}
                                        ...
                                        <div className="flex gap-3">
                                            <button 
                                                ...
                                                className="flex-1 bg-slate-900 text-white px-6 py-4 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200"
                                            >
                                                View Details
                                            </button>
                                            <button 
                                                onClick={() => handleDelete(ticket.id)}
                                                className="bg-rose-50 text-rose-500 px-6 py-4 rounded-2xl text-xs font-black hover:bg-rose-500 hover:text-white transition-all"
                                            >
                                                Delete
                                            </button>
                                        </div>
                        ...
                        <div className="col-span-full py-32 text-center bg-white rounded-[60px] border-4 border-dashed border-slate-50">
                            <p className="text-slate-300 font-black text-2xl uppercase tracking-tighter">No Records Found</p>
                        </div>
                    )}
                </div>
            </div>
